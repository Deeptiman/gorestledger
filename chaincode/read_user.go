...
func (t *HelloWorldServiceChaincode) readAllUser(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if err != nil {
		fmt.Println("Unable to retrieve the list of resource in the ledger")
	...
	for iterator.HasNext() {
		...
		if errIt != nil {
			fmt.Println("Unable to retrieve a user in the ledger")
	...
	//fmt.Println("Size of allUsersData : "+len(allUsersData))

	allUsersAsByte, err := objectToByte(allUsersData)
	if err != nil {
		fmt.Println("Unable to convert the users list to byte")
