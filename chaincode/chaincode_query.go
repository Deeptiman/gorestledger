...
func getFromLedger(stub shim.ChaincodeStubInterface, key string, id string, result interface{}) error {
	...
	if err != nil {
		fmt.Println("getFromLedger :: unable to retrieve the object in the ledger --- " + err.Error())
		return fmt.Errorf("unable to retrieve the object in the ledger: %v", err)
	}
	if resultAsByte == nil {
		fmt.Println("getFromLedger :: the object doesn't exist in the ledger")
		return fmt.Errorf("the object doesn't exist in the ledger")
	}
	err = byteToObject(resultAsByte, result)
	if err != nil {
		fmt.Println("getFromLedger :: unable to convert the result to object --- " + err.Error())
